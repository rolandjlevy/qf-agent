...
Without these, trade-specific quality rests on the model's general knowledge: Phase A and Phase B get little more than the trade name. A pack is a reviewed list of a trade's common jobs, each with diagnostic questions, typical materials per variant, standard assumptions and exclusions, and real-world pitfalls. Plumber is the pilot (`lib/trade-knowledge/plumber.js`, 8 jobs); the others follow once it's proven. Full design, eval results and how to add a trade: `docs/PHASE_3C_TRADE_KNOWLEDGE.md`.
...
- **Guardrails in tests:** `lib/trade-knowledge/plumber.test.js` checks that every pack material passes `isRejectedLabel`, that no pack text contains compliance or approval wording (including WRAS), and that questions follow the key-question rules (the form adds "Other"/"Not sure" itself).
...
`agent.test.js` (vitest) tests `agent.js`'s loop directly, mocking `lib/anthropic-client.js` — architecture-agnostic, so it applies regardless of how the CLI or web UI evolve. `lib/pricing/` has its own suite (`index.test.js`, `CachedPriceSearchProvider.test.js`, `providers/SerperPriceSearchProvider.test.js`, `cache/DbCacheStore.test.js`) covering provider selection, caching behaviour, and Serper response mapping/mock mode. `lib/quote-materials.test.js` and `tools/{draft-section,identify-materials}.test.js` cover their respective modules. `lib/trade-knowledge/{index,plumber}.test.js` cover the Phase 3c packs (gating, formatters, and the pack content's own guardrails); live-API quality is measured separately by `npm run eval` (see "Trade knowledge packs" above). `lib/material-rules.test.js` and `lib/propose-materials.test.js` cover the materials-refinement flow's Phase A (see "Materials refinement" above); `app/api/quote/route.js`, `app/quote/new/page.js`, `app/materials-refinement.js`, and `lib/actions/log-refinement.js` have none, consistent with "Known caveats" below. The `.claude/skills/test-impact` skill (`npm run test:impact`) diffs the working tree and reports which tests a change touches, or flags a change with no coverage at all — use it before wrapping up a feature. See its `SKILL.md` for the full workflow.
