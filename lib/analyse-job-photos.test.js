...
describe('analyseJobPhotos response handling', () => {
  ...
  it('filters compliance, safety and asbestos judgements from observations and resolved topics', async () => {
    ...
    expect(result.unclear.map((u) => u.question)).toEqual(['Has the textured coating been tested for asbestos?']);
  });
  ...
  it('caps observations and topics even when the model returns more', async () => {
    ...
    expect(result.unclear).toEqual(topics.slice(0, 6).map((t) => ({ topic: t, question: t, options: [] })));
  });
