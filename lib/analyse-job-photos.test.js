...
describe('analyseJobPhotos response handling', () => {
  it('returns normalized observations, resolved and unclear topics', async () => {
    createMessage.mockResolvedValueOnce(
      jsonResponse({
        ...
        unclear: ['earthing arrangement', 42],
      }),
    );
  ...
  it('filters compliance, safety and asbestos judgements from observations and resolved topics', async () => {
    createMessage.mockResolvedValueOnce(
      jsonResponse({
        ...
        unclear: ['whether the artex has been tested for asbestos'],
      }),
    );
    ...
    expect(result.unclear).toEqual(['whether the artex has been tested for asbestos']);
  });
  ...
  it('caps observations and topics even when the model returns more', async () => {
    ...
    expect(result.unclear).toEqual(topics.slice(0, 6));
  });
