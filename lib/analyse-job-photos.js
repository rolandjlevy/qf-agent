...
- "unclear": at most ${MAX_TOPICS} short topics that most matter for this job's quote but the photos can't show (e.g. "condition of the wiring behind the walls"), most important first.
- Asbestos: never state whether anything contains it. Only when a textured ceiling or wall coating (artex-style) is actually visible, add "whether the textured coating has been tested for asbestos" to "unclear". Otherwise don't mention asbestos at all.
</instructions>

<output_format>
Return ONLY a JSON object, no markdown fences, no preamble:
{ "photos": [ { "imageIndex": 1, "kind": "site|reference|irrelevant" } ], "observations": [ { "imageIndex": 1, "observation": "string", "confidence": "high|medium|low" } ], "resolved": ["string"], "unclear": ["string"] }
</output_format>

<example>
Shows the format and level of detail only; never copy its content.
Job: "Electrician — swap old fusebox for a new consumer unit"
{ "photos": [ { "imageIndex": 1, "kind": "site" }, { "imageIndex": 2, "kind": "site" } ], "observations": [ { "imageIndex": 1, "observation": "Wylex fuse box with 6 rewireable fuse carriers, no RCD", "confidence": "high" }, { "imageIndex": 2, "observation": "Meter tails appear to be older, thinner cable than modern 25mm tails", "confidence": "low" } ], "resolved": ["existing consumer unit type", "number of circuits"], "unclear": ["condition of the wiring behind the walls", "earthing arrangement at the incoming supply"] }
</example>`
}
...
export async function analyseJobPhotos({ trade, jobDescription, images, signal }) {
  ...
  return { photos, observations, resolved, unclear: normalizeTopics(parsed.unclear) }
}
