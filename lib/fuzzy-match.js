...
// Job descriptions and identify_materials output routinely use the word
// form of small counts ("single gang", "double gang") while real supplier
// product titles almost always use the digit form ("1-Gang", "2-Gang") —
// confirmed in production: "single gang 1-way light switch" scored only 33
// against the exact matching product "1-Gang 1-Way Light Switch" purely
// because "single"/"1" and "double"/"2" glue to different tokens
// ("gang" vs "1gang"), losing several points of word-overlap on a term the
// two names actually agree on. Map the word forms to digits before gluing
// so both phrasings land on the identical token.
const WORD_COUNTS = {
  one: '1', single: '1',
  two: '2', double: '2',
  three: '3', triple: '3',
...
function glueCountedTerms(normalized) {
  const withDigitCounts = normalized.replace(
    /\b(one|single|two|double|three|triple|four|quad)\s+(gang|way)\b/g,
    (_match, word, unit) => `${WORD_COUNTS[word]}${unit}`,
  )
  return withDigitCounts.replace(/\b(\d+)\s+(gang|way)\b/g, '$1$2')
}
