...
describe('createCachedProvider', () => {
  ...
  it('sends the original, unsorted query to the provider', async () => {
    const search = vi.fn(async () => ({ products: [makeProduct()], providerName: 'fake', fetchedAt: Date.now(), fromCache: false }))
    const cached = createCachedProvider({ name: 'fake', search }, cacheStore, 3600)

    await cached.search('Plasterboard 12.5mm')

    expect(search).toHaveBeenCalledWith('Plasterboard 12.5mm', {})
  })
...
describe('makeCacheKey', () => {
  ...
  it('is stable across word-order differences', () => {
    expect(makeCacheKey('Plasterboard 12.5mm')).toBe(makeCacheKey('12.5mm plasterboard'))
    expect(makeCacheKey('plasterboard 12.5mm')).not.toBe(makeCacheKey('plasterboard 15mm'))
  })
