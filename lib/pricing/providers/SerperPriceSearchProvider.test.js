...
describe('SerperPriceSearchProvider (mock mode)', () => {
  ...
  it('filters to the requested merchant, returning fewer than maxResults when fewer exist', async () => {
    const provider = new SerperPriceSearchProvider()
    // Fixture data has exactly 2 Screwfix products.
    const result = await provider.search('paint', { merchant: 'Screwfix' })
    expect(result.products).toHaveLength(2)
    for (const product of result.products) {
      expect(product.merchant).toBe('Screwfix')
    }
  })
})
...
describe('SerperPriceSearchProvider (real API path, fetch mocked)', () => {
  ...
  // Regression coverage for a real, observed flake: an identical live query
  // returned 0 raw results three times in a row, then a full page moments
  // later, with nothing about the request different — see
  // ZERO_RESULT_RETRY_DELAY_MS's comment.
  it('retries once when a successful response comes back with zero raw results, and returns the retry\'s products', async () => {
    const fetchSpy = vi
      .spyOn(globalThis, 'fetch')
      .mockResolvedValueOnce({ ok: true, status: 200, json: async () => ({ shopping: [] }) })
      .mockResolvedValueOnce({
        ok: true,
        status: 200,
        json: async () => ({ shopping: [{ title: 'Good Paint', price: '£19.99', source: 'Screwfix', link: 'https://x.test/1' }] }),
      })
    const provider = new SerperPriceSearchProvider()
    ...
    expect(result.products).toHaveLength(1)
    expect(result.products[0].title).toBe('Good Paint')
  })

  it('does not retry more than once when results stay empty — returns an empty list after exactly 2 calls', async () => {
    const fetchSpy = vi.spyOn(globalThis, 'fetch').mockResolvedValue({ ok: true, status: 200, json: async () => ({ shopping: [] }) })
    const provider = new SerperPriceSearchProvider()

    const result = await provider.search('paint')

    expect(fetchSpy).toHaveBeenCalledTimes(2) // initial + exactly one zero-result retry, not a loop
  ...
  it('over-fetches (num=40) and requests the plain maxResults page when no merchant filter is given', async () => {
    // Non-empty shopping so the zero-result retry (see
    // ZERO_RESULT_RETRY_DELAY_MS) never fires — this test is only about the
    // `num` param, not retry behaviour, and an extra retry call would shift
    // fetchSpy.mock.calls' indices out from under the assertions below.
    const fetchSpy = vi.spyOn(globalThis, 'fetch').mockResolvedValue({
      ok: true,
      status: 200,
      json: async () => ({ shopping: [{ title: 'x', price: '£1.00', source: 'Screwfix', link: 'https://x.test/1' }] }),
    })
    const provider = new SerperPriceSearchProvider()

    await provider.search('paint', { merchant: 'Screwfix' })
    expect(JSON.parse(fetchSpy.mock.calls[0][1].body).num).toBe(40)

    await provider.search('paint', { maxResults: 5 })
    expect(JSON.parse(fetchSpy.mock.calls[1][1].body).num).toBe(5)
  })

  it('filters a wide raw page down to just the requested merchant, capped at maxResults', async () => {
    // 15 raw results: 12 Screwfix, 3 Toolstation — more than maxResults
    // (default 10) worth of Screwfix, so the filtered+capped result should
    // fill a full page of 10, not come back short because Toolstation
    // entries occupied slots ahead of the filter.
    const shopping = [
      ...Array.from({ length: 12 }, (_, i) => ({
        title: `Screwfix item ${i}`,
        price: '£9.99',
        source: 'Screwfix',
        link: `https://x.test/sf-${i}`,
        productId: `sf-${i}`,
      })),
      ...Array.from({ length: 3 }, (_, i) => ({
        title: `Toolstation item ${i}`,
        price: '£8.99',
        source: 'Toolstation',
        link: `https://x.test/ts-${i}`,
        productId: `ts-${i}`,
      })),
    ]
    vi.spyOn(globalThis, 'fetch').mockResolvedValue({
  ...
  it('returns fewer than maxResults when fewer than maxResults match the merchant', async () => {
    const shopping = [
      { title: 'Screwfix item', price: '£9.99', source: 'Screwfix', link: 'https://x.test/sf-0', productId: 'sf-0' },
      { title: 'Toolstation item', price: '£8.99', source: 'Toolstation', link: 'https://x.test/ts-0', productId: 'ts-0' },
    ]
    vi.spyOn(globalThis, 'fetch').mockResolvedValue({
      ok: true,
      status: 200,
    ...
    expect(result.products).toHaveLength(1)
    expect(result.products[0].merchant).toBe('Screwfix')
  })
})
