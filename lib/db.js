...
// confirmed via production logs (ENOENT on lib/schema.sql on Vercel).
const SCHEMA = `
...
async function ensureContentColumn(db) {
  const info = await db.execute('PRAGMA table_info(generated_quotes)')
  const hasContent = info.rows.some((row) => row.name === 'content')
  if (!hasContent) {
    await db.execute('ALTER TABLE generated_quotes ADD COLUMN content TEXT')
  }
}
...
export async function getDb() {
  const db = getClient()
  if (!schemaReady) {
    schemaReady = db.executeMultiple(SCHEMA).then(() => ensureContentColumn(db))
  }
  await schemaReady
  return db
}
...
export async function insertGeneratedQuote({ job_description, output_path, content, tool_call_log }) {
  const db = await getDb()
  const result = await db.execute({
    sql: `INSERT INTO generated_quotes (job_description, output_path, content, generated_at, tool_call_log)
          VALUES (:job_description, :output_path, :content, :generated_at, :tool_call_log)`,
    args: {
      job_description,
