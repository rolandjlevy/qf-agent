...
// Guarded to non-terminal statuses, same as abortQuoteRun below: a cross-
// instance cancel (POST /api/quote/[runId]/cancel) can write 'aborted' to
// this row while this run's own instance hasn't heard yet, and its in-flight
// turn can still emit one or more further onStep calls before that — each
// one is a call to this function. Without the guard, "status = 'running'"
// unconditionally overwrites the 'aborted' state, and the row flickers back
// to 'running' until this instance's own watchdog next polls and catches up.
...
// A trader's per-line overrides (chosen product, quantity, status) per
// material line — see quote_line_prices in lib/schema.sql. Additive to
// generated_quotes, never overwrites it.
export async function getQuoteLinePrices(quoteId) {
  return getClient()`SELECT material_name, product, quantity_override, status, name_override, selected_at FROM quote_line_prices WHERE quote_id = ${quoteId}`
}

// Each of the three upserts below only ever touches its own column (plus
// selected_at as a shared "last touched" timestamp) via ON CONFLICT DO
// UPDATE — a quantity edit must never clobber an already-selected price, a
// price selection must never reset a 'saved_for_later'/'deleted' status back
// to 'active', and so on. The columns not mentioned in each INSERT's column
// list take their schema defaults (product NULL, status 'active') on a
// fresh row and are left untouched by DO UPDATE SET on an existing one.
// nameOverride is only set (never cleared) here — omitting it must leave a
// line's existing display-name override untouched, same "don't clobber
// unrelated fields" rule the comment above describes.
export async function upsertQuoteLinePrice(quoteId, materialName, product, nameOverride) {
  const sql = getClient()
  if (nameOverride) {
    await sql`
      INSERT INTO quote_line_prices (quote_id, material_name, product, name_override, selected_at)
      VALUES (${quoteId}, ${materialName}, ${JSON.stringify(product)}, ${nameOverride}, ${new Date().toISOString()})
      ON CONFLICT (quote_id, material_name) DO UPDATE SET product = excluded.product, name_override = excluded.name_override, selected_at = excluded.selected_at
    `
    return
  }
  await sql`
...
export async function upsertQuoteLineQuantity(quoteId, materialName, quantity) {
  await getClient()`
    INSERT INTO quote_line_prices (quote_id, material_name, quantity_override, selected_at)
    VALUES (${quoteId}, ${materialName}, ${quantity}, ${new Date().toISOString()})
    ON CONFLICT (quote_id, material_name) DO UPDATE SET quantity_override = excluded.quantity_override, selected_at = excluded.selected_at
  `
}

export async function upsertQuoteLineStatus(quoteId, materialName, status) {
  await getClient()`
    INSERT INTO quote_line_prices (quote_id, material_name, status, selected_at)
    VALUES (${quoteId}, ${materialName}, ${status}, ${new Date().toISOString()})
    ON CONFLICT (quote_id, material_name) DO UPDATE SET status = excluded.status, selected_at = excluded.selected_at
  `
}

// Analytics for the materials-refinement step — see material_refinement_events
// in lib/schema.sql. Written as one Postgres transaction (the Neon HTTP
// driver's sql.transaction, a single round trip) so a partial write never
// happens; a failure here must never break quote generation — see the caller,
// lib/actions/log-refinement.js.
export async function logRefinementEvents(events) {
  ...
  await client.transaction(
    events.map(
      (e) => client`
        INSERT INTO material_refinement_events (session_id, job_description, label, source, action, created_at)
        VALUES (${e.session_id}, ${e.job_description}, ${e.label}, ${e.source}, ${e.action}, ${createdAt})
      `,
    ),
  )
}
