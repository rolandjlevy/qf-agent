...
// Replaces (rather than accumulates) the price for one (material, supplier)
// pair — a re-scrape should reflect the current listing, not build up price
// history. Delete-then-insert since there's no natural unique constraint to
// ON CONFLICT against without adding one.
export async function upsertScrapedPrice({ material_name, supplier, price, sku, product_url }) {
  const client = getClient()
  await client`DELETE FROM scraped_prices WHERE material_name = ${material_name} AND supplier = ${supplier}`
  await client`
    INSERT INTO scraped_prices (material_name, supplier, price, sku, product_url, scraped_at)
    VALUES (${material_name}, ${supplier}, ${price}, ${sku ?? null}, ${product_url ?? null}, ${new Date().toISOString()})
  `
}

export async function getScrapedPrices(materialName) {
  return getClient()`SELECT * FROM scraped_prices WHERE material_name = ${materialName} ORDER BY price ASC`
}

// Called when a re-scrape finds no confident match for a (material,
// supplier) pair, so a stale row from a previous run (e.g. one cached
// before a matching-logic fix, or a listing that's since disappeared)
// doesn't linger forever just because this run didn't happen to overwrite
// it — "no confident match" should mean the cache reflects that, not
// whatever was left over from last time.
export async function deleteScrapedPrice(materialName, supplier) {
  await getClient()`DELETE FROM scraped_prices WHERE material_name = ${materialName} AND supplier = ${supplier}`
}
