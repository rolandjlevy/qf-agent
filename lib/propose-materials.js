...
export async function proposeMaterials({ trade, jobDescription, priorQuestions = [], photoFindings }) {
  ...
  if (priorQuestions.some((qa) => answerSignalsInspectionNeeded(qa?.answer))) {
    return { materials: [] }
  }
...
<job_description>
${jobDescription}
</job_description>${formatPhotoFindingsForPhaseA(photoFindings)}${qaBlock}
